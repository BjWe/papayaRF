...
void handleListDevicesConf() {
  ...
  message += "<tr><td>Serial</td><td>Name</td><td>Key</td><td>Nextcode</td></tr>";
  ...
  while(cfgStore.next(&info)){
    ...
    message += "<tr><td>" + String(info.serial, HEX) + "</td><td>" + String(info.name)+ "</td><td>" + keystr + "</td><td>" + String(info.nextcode, HEX) + "</td></tr>"; 
  }

  message += "</table><br /><span>Registration is " + String(allowRegister ? "enabled" : "disabled");
...
void processBasicMsg(uint8_t (&rx)[SI4432::MAX_PACKET_LENGTH]){
  rf_packet_basic_t msg;
          ...
          if(cfgStore.fetch(msg.header.serial, &info)){
            ...
            uint8_t crc = crc8maxim(&decrypted_payload , sizeof(rf_packet_payload_basic_t) - 1);
            if(crc == decrypted_payload.crc){
              Serial.println("CRC OK");
            } else {
              Serial.println("CRC missmatch - 0x" + String(crc, HEX) + " <> 0x" + String(decrypted_payload.crc, HEX));  
            }
            ...
            if(checkNextcode(info.nextcode, decrypted_payload.nextcode, 50) == 1){
              ...
              if(btn_cfg_pos < 0){
                ...
                for(uint8_t i = 0; i < 32; i++){
                  if(info.io[i][0] == 0xFF){
                    Serial.println("IO Pos 0x" + String(i, HEX) + " is free");
                    info.io[i][0] = (uint8_t)decrypted_payload.data;
                    info.io[i][1] = ITEMTYPE_BUTTON;
                    //mqtt.sendIOConfigStatus(rfinfo.serial, rfinfo.io[btn_cfg_pos][0]);
                    mqtt.sendIOConfigStatus(info.serial, info.io[i][0]);
                    mqtt.sendIOOnline(info.serial, "Online", info.io[i][0]);
                    btn_cfg_pos = i;
                    break;
              ...
              switch(decrypted_payload.msgtype){
                case BUTTON_DOWN: {
                  mqtt.sendIOStatus(info.serial, "ON", info.io[btn_cfg_pos][0]);
                  break;  
                }
                case BUTTON_UP: {
                  mqtt.sendIOStatus(info.serial, "OFF", info.io[btn_cfg_pos][0]);
                  break;  
                }
              }
...
void processRekeyMsg(uint8_t (&rx)[SI4432::MAX_PACKET_LENGTH]){
  if(allowRegister){
    ...
    String filename = "/devices/" + String(msg.header.serial, HEX) + ".conf"; 
    ...
    if(LittleFS.exists(filename)){
      ...
      String(msg.header.serial, HEX).toCharArray(rfinfo.name, sizeof(rfinfo.name));
